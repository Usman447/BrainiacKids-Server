    ...
    internal class PendingMessage
    {
        ...
        internal void TrySend()
        {
            if (sendAttempts >= MaxSendAttempts)
            {
                ...
                if (NetworkLogger.IsWarningLoggingEnabled)
                {
                    MessageHeader header = (MessageHeader)data[0];
                    if (header == MessageHeader.Reliable)
                        NetworkLogger.Log(LogType.Warning, connection.Peer.LogName, $"No ack received for {header} message (ID: {Converter.ToUShort(data, 3)}) after {sendAttempts} {Helper.CorrectForm(sendAttempts, "attempt")}, delivery may have failed!");
                    else
                        NetworkLogger.Log(LogType.Warning, connection.Peer.LogName, $"No ack received for internal {header} message after {sendAttempts} {Helper.CorrectForm(sendAttempts, "attempt")}, delivery may have failed!");
                }
