...
namespace Network
{
    ...
    public abstract class Connection
    {
        ...
        public int TimeoutTime { get; set; } = 20000;
        ...
        internal bool ReliableHandle(ushort sequenceId)
        {
            ...
            if (sequenceGap > 0)
            {
                ...
                if (sequenceGap > 64)
                    NetworkLogger.Log(LogType.Warning, Peer.LogName, $"The gap between received sequence IDs was very large ({sequenceGap})! If the connection's packet loss, latency, or your send rate of reliable messages increases much further, sequence IDs may begin falling outside the bounds of the duplicate filter.");
        ...
        internal void HandleWelcomeResponse(Message message)
        {
            ...
            if (Id != id)
                NetworkLogger.Log(LogType.Error, Peer.LogName, $"Client has assumed ID {id} instead of {Id}!");
