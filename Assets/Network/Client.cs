...
namespace Network
{
    ...
    public class Client : Peer
    {
        ...
        {
            transport = new Transports.Tcp.TcpClient();
        }
        ...
        /// <remarks>
        ///   <para>Network's default transport expects the host address to consist of an IP and port, separated by a colon. For example: <c>127.0.0.1:7777</c>. If you are using a different transport, check the relevant documentation for what information it requires in the host address.</para>
        ...
        public bool Connect(string hostAddress, int maxConnectionAttempts = 5, byte messageHandlerGroupId = 0, Message message = null, bool useMessageHandlers = true)
        {
            ...
            {
                NetworkLogger.Log(LogType.Error, LogName, connectError);
            ...
            Heartbeat();
            NetworkLogger.Log(LogType.Info, LogName, $"Connecting to {connection}...");
            return true;
        }
        ...
        protected override void Handle(Message message, MessageHeader header, Connection connection)
        {
            switch (header)
            {
                ...
                case MessageHeader.ClientDisconnected:
                    ...
                    break;
                default:
                    NetworkLogger.Log(LogType.Warning, LogName, $"Unexpected message header '{header}'! Discarding {message.WrittenLength} bytes.");
                    break;
            }
        ...
        {
            NetworkLogger.Log(LogType.Info, LogName, "Connected successfully!");
        ...
        protected virtual void OnConnectionFailed(RejectReason reason, Message message = null)
        {
            ...
            NetworkLogger.Log(LogType.Info, LogName, $"Connection to server failed: {reasonString}.");
        ...
        protected virtual void OnDisconnected(DisconnectReason reason, Message message)
        {
            ...
            NetworkLogger.Log(LogType.Info, LogName, $"Disconnected from server: {reasonString}.");
        ...
        {
            NetworkLogger.Log(LogType.Info, LogName, $"Client {clientId} connected.");
        ...
        {
            NetworkLogger.Log(LogType.Info, LogName, $"Client {clientId} disconnected.");
