...
namespace Network
{
    ...
    public class InsufficientCapacityException : Exception
    {
        ...
        public readonly Message NetworkMessage;
    ...
    public class InvalidHandlerSignatureException : Exception
    {
        ...
        private static string GetErrorMessage(Type declaringType, string handlerMethodName)
        {
            return $"'{declaringType.Name}.{handlerMethodName}' doesn't match any acceptable message handler method signatures! Server message handler methods should have a 'ushort' and a '{nameof(Network.Message)}' parameter, while client message handler methods should only have a '{nameof(Network.Message)}' parameter.";
        }
    }
