...
namespace Network
{
    ...
    public class Server : Peer
    {
        ...
        /// <param name="logName">The name to use when logging messages via <see cref="NetworkLogger"/>.</param>
        ...
        /// <summary>Handles initial setup using the built-in TCP transport.</summary>
        /// <param name="logName">The name to use when logging messages via <see cref="NetworkLogger"/>.</param>
        public Server(string logName = "SERVER") : base(logName)
        {
            transport = new Transports.Tcp.TcpServer();
            pendingConnections = new List<Connection>();
            clients = new Dictionary<ushort, Connection>();
            timedOutClients = new List<Connection>();
        }
        ...
        public void Start(ushort port, ushort maxClientCount, byte messageHandlerGroupId = 0, bool useMessageHandlers = true)
        {
            ...
            Heartbeat();
            IsRunning = true;
            NetworkLogger.Log(LogType.Info, LogName, $"Started on port {port}.");
        }
        ...
        public void Accept(Connection connection)
        {
            if (pendingConnections.Remove(connection))
                ...
                NetworkLogger.Log(LogType.Warning, LogName, $"Couldn't accept connection from {connection} because no such connection was pending!");
        }
        ...
        public void Reject(Connection connection, Message message = null)
        {
            if (pendingConnections.Remove(connection))
                ...
                NetworkLogger.Log(LogType.Warning, LogName, $"Couldn't reject connection from {connection} because no such connection was pending!");
        }
        ...
        private void Reject(Connection connection, RejectReason reason, Message rejectMessage = null)
        {
            ...
            NetworkLogger.Log(LogType.Info, LogName, $"Rejected connection from {connection}: {reasonString}.");
        }
        ...
        protected override void Handle(Message message, MessageHeader header, Connection connection)
        {
            switch (header)
            {
                ...
                case MessageHeader.Welcome:
                    ...
                    break;
                default:
                    NetworkLogger.Log(LogType.Warning, LogName, $"Unexpected message header '{header}'! Discarding {message.WrittenLength} bytes received from {connection}.");
                    break;
            }
        ...
        public void DisconnectClient(ushort id, Message message = null)
        {
            if (clients.TryGetValue(id, out Connection client))
            {
                ...
                NetworkLogger.Log(LogType.Warning, LogName, $"Couldn't disconnect client {id} because it wasn't connected!");
        }
        ...
        public void DisconnectClient(Connection client, Message message = null)
        {
            if (clients.ContainsKey(client.Id))
            {
                ...
                NetworkLogger.Log(LogType.Warning, LogName, $"Couldn't disconnect client {client.Id} because it wasn't connected!");
        }
        ...
        public void Stop()
        {
            ...
            StopTime();
            IsRunning = false;
            NetworkLogger.Log(LogType.Info, LogName, "Server stopped.");
        }
        ...
        private ushort GetAvailableClientId()
        {
            ...
            NetworkLogger.Log(LogType.Error, LogName, "No available client IDs, assigned 0!");
            return 0;
        }
        ...
        {
            NetworkLogger.Log(LogType.Info, LogName, $"Client {client.Id} ({client}) connected successfully!");
        ...
        protected virtual void OnMessageReceived(Message message, Connection fromConnection)
        {
            ...
            if (useMessageHandlers)
            {
                if (messageHandlers.TryGetValue(messageId, out MessageHandler messageHandler))
                    ...
                    NetworkLogger.Log(LogType.Warning, LogName, $"No message handler method found for message ID {messageId}!");
            }
        }
        ...
        protected virtual void OnClientDisconnected(Connection connection, DisconnectReason reason)
        {
            ...
            NetworkLogger.Log(LogType.Info, LogName, $"Client {connection.Id} ({connection}) disconnected: {reasonString}.");
